...
function StackedBarChart({ bins, binSeconds, series }: StackedBarChartProps) {
  ...
  const hoveredTopPct = hovered
    ? ((padding.top +
        innerH -
        (series.reduce(
          (s, sr) => s + (hovered[sr.key as keyof typeof hovered] as number),
          0,
        ) /
          maxTotal) *
          innerH) /
          ...
          <div className="mt-1 grid gap-0.5">
            ...
            <div className="mt-0.5 flex items-center justify-between gap-3 border-t border-border/60 pt-0.5 font-medium">
              ...
              <span className="tabular-nums">
                {hovered.totalTokens.toLocaleString()}
              </span>
            </div>
          </div>
        </ChartTooltip>
...
export function TokensUsagePanel({ projectId, environmentId }: Props) {
  ...
  useEffect(() => {
    ...
    // refresh isn't a cascading render, and drop it after unmount.
  ...
  return (
    <div className="grid gap-8">
      ...
      <Section
        ...
        description="Stacked: input · output · reasoning · cached input."
      >
        <div className="rounded-lg border border-border bg-card p-3">
          <StackedBarChart
            bins={bins}
            binSeconds={stats?.binSeconds ?? 0}
            series={TOKEN_SERIES}
          />
